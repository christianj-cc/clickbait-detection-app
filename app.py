...
:root {
    ...
    --border-bright: rgba(230, 230, 230, 0.35);
...
/* Tighten the automatic vertical rhythm Streamlit adds between
   stacked elements in the main content column. */
...
.stButton > button:disabled p::after {
    content: '';
    display: inline-block;
    width: 1.1em;
    text-align: left;
    animation: btn-dots 1.2s steps(4, end) infinite;
}
 
@keyframes btn-pulse {
    0%, 100% { box-shadow: 0 4px 20px rgba(56,189,248,0.30); }
    50% { box-shadow: 0 4px 30px rgba(56,189,248,0.65); }
}
...
with st.sidebar:
    ...
    md(
        ...
        '<span class="spec-chip">BiLSTM · 64 units</span>',
        '<span class="spec-chip">Self-Attention</span>',
        '</div>',
...
with center:
    ...
    # Dynamic Analyze button: while processing, it renders disabled,
    # relabeled to "Analyzing...", and gets a pulsing glow + animated
    # ellipsis via CSS so the user gets immediate, obvious feedback
    # that the system is working instead of a static, unchanged button.
    button_label = "Analyzing..." if st.session_state.processing else "Analyze Headline"
    ...
    # step indicator and the button immediately reflect "Analyzing..."
    # before the (fast) inference actually runs.
    # -------------------------------------------------------------------
